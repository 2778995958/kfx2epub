...
本ツールは現在、日本の Kindle 書籍のみを対象として使用・検証しています。他の地域または他言語の KFX 書籍については、正常な変換や理想的なレイアウト結果を保証しません。
...
- ドラッグ＆ドロップで実行した場合も、出力先は `convert_kfx2epub.py` と同じ場所にある `output_epub` に固定されます。
- 出力 EPUB を `[著者] タイトル.epub` 形式で自動命名します。
- ファイル名に使えない文字を全角の安全な文字に自動変換します。
- EPUB 内部のフォルダー、XHTML、OPF、CSS、画像ファイル名を、`item/standard.opf`、`navigation-documents.xhtml`、`p-cover.xhtml`、`p-toc.xhtml`、`p-fmatter-0001.xhtml`、`p-colophon.xhtml`、`book-style.css`、`style-reset.css`、`image/cover.ext`、`image/i-0000.ext`、`image/p-0000.ext` のような規則的な名前にします。
...
この場合も、出力先は `convert_kfx2epub.py` と同じ場所にある `output_epub` になります。
...
- spine 内で `p-toc.xhtml` を判定できる場合、表紙と目次の間にある XHTML は spine 順に `item/xhtml/p-fmatter-0001.xhtml` から命名されます。
- `p-toc.xhtml` より後の通常本文ページは spine 順に `item/xhtml/p-0001.xhtml` から命名されます。
- toc がない場合、通常本文ページは spine 順に `item/xhtml/p-0001.xhtml` からの `p-` 系列を使用します。
...
- 挿絵型の画像は `item/image/i-0000.ext` から順番に命名されます。
- その他の通常画像は `item/image/p-0000.ext` から順番に命名されます。
