...
- EPUB 内部のフォルダー、XHTML、OPF、CSS、画像ファイル名を、`item/standard.opf`、`navigation-documents.xhtml`、`p-cover.xhtml`、`p-toc.xhtml`、`p-fmatter-001.xhtml`、`p-colophon.xhtml`、`book-style.css`、`style-reset.css`、`image/cover.ext`、`image/i-000.ext`、`image/p-000.ext` のような規則的な名前にします。
...
- 挿絵型の画像は `item/image/i-000.ext` から順番に命名されます。
- その他の通常画像は `item/image/p-000.ext` から順番に命名されます。
