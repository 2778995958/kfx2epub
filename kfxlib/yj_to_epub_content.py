...
class KFX_EPUB_Content(object):
    ...
    def process_content(self, content, parent, book_part, writing_mode, content_layout=None, fixed_height=None,
        ...
        if content_type == "$269":
            ...
            if "$152" in content:
                ...
                for col_fmt in content.pop("$152"):
                    ...
                    col_fmt.pop("box_align", None)
                    col_fmt.pop("fit_width", None)

                    col_style = self.convert_yj_properties(col_fmt)
                    self.fix_vertical_align_properties(col_elem, col_style)
                    self.add_style(col_elem, col_style)
        ...
        if (dropcap_lines and not dropcap_chars) or dropcap_chars < 0 or dropcap_lines < 0:
            ...
            if content_type not in ["$269", "$277", "$270", "$271"]:
                log.warning("%s id %s has unexpected dropcap in %s" % (self.content_context, location_id, content_type))
            ...
            style_events.insert(0, dropcap_style_event)

        for style_event in style_events:
            event_offset = style_event.pop("$143")
            event_length = style_event.pop("$144")
            is_dropcap = "$125" in style_event

            event_elem = self.find_or_create_style_event_element(content_elem, event_offset, event_length, is_dropcap)
            ...
            if "$757" in style_event:
                ...
                for ruby_id_entry in ruby_id_list:
                    ...
                    rb_elem = self.find_or_create_style_event_element(event_elem, ruby_offset, ruby_length, False)
                    if rb_elem is None:
                        break
        ...
        if render == "$283":
            ...
            if content_elem.tag in ["a", "audio", "img", SVG, "video"] and content_style.get("position") != "fixed":
                ...
                if "float" in container_style:
                    width_or_height = "width" if writing_mode != "vertical-rl" else "height"
    ...
    def locate_offset(self, root, offset_query, split_after=False, zero_len=False, is_dropcap=False):
        if self.DEBUG:
            log.debug("locating offset %d %sin %s" % (offset_query, "dropcap " if is_dropcap else "", etree.tostring(root)))

        result = self.locate_offset_in(root, offset_query, split_after, zero_len, is_dropcap)
        ...
        if is_dropcap:
            log.warning("Dropcap position %d exceeds content: %s" % (offset_query, etree.tostring(root)))
            return etree.SubElement(root, "span")

        log.error("locate_offset failed to find offset %d (remaining=%d, split_after=%s, dropcap=%s) in %s" % (
                            offset_query, result, str(split_after), str(is_dropcap), etree.tostring(root)))

        return None

    def locate_offset_in(self, elem, offset_query, split_after, zero_len, is_dropcap):
        ...
        if elem.tag == "span":
            ...
            if is_dropcap and self.get_style(elem).get("float"):
                scan_children = False

            elif elem.tag in {"img", SVG, MATH} or self.get_style(elem).get("-kfx-render") == "inline":
                scan_children = False
        ...
        if scan_children:
            for e in elem.iterfind("*"):
                result = self.locate_offset_in(e, offset_query, split_after, zero_len, is_dropcap)
    ...
    def find_or_create_style_event_element(self, content_elem, event_offset, event_length, is_dropcap):

        if event_length <= 0:
            raise Exception("%s style event has length: %s" % (self.content_context, event_length))

        first = self.locate_offset(content_elem, event_offset, split_after=False, is_dropcap=is_dropcap)
        if first is None:
            return None

        last = self.locate_offset(content_elem, event_offset + event_length - 1, split_after=True, is_dropcap=is_dropcap)
        ...
        if first_parent != last_parent:
            ...
            for try_first in firsts:
                ...
                log.warning("%s style event first and last have different parents: offset %d len %d: %s" % (
